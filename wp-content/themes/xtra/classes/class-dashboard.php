	...
	class Xtra_Dashboard extends Xtra_Theme {
			...
			if ( ! isset( $this->disable[ 'menu' ] ) || $this->is_free ) {
				...
				$this->plugins 	= apply_filters( 'xtra_plugins_list', [
					'codevz-plus' 	=> [
						...
						'source' 			=> get_template_directory_uri() . '/plugins/codevz-plus.zip',
						'required' 			=> true,
		...
		public function plugins() {
			...
			foreach( $this->plugins as $slug => $plugin ) {
				...
				//if ( ! self::$premium && Xtra_Theme::contains( $slug, [ 'js_composer', 'revslider', 'codevz-plus' ] ) ) {
					//continue;
				//}
		...
		public function xtra_page_importer() {
			...
			if ( ! empty( $_POST[ 'url' ] ) ) {
				...
				if ( filter_var( $url, FILTER_VALIDATE_URL ) === FALSE || ! self::contains( $url, 'xtratheme' ) ) {
