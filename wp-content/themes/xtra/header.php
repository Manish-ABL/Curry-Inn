...
<?php 
	...
	$cover_than_header = Xtra_Theme::option( 'cover_than_header' . $option_cpt );
	...
	// Inner layout
	echo '<div class="inner_layout' . ( $header ? '' : ' cz-no-header' ) . esc_attr( $cover_than_header ? ' ' . $cover_than_header : '' ) . '"' . esc_attr( $il_width ) . '><div class="cz_overlay" aria-hidden="true"></div>';
	...
	if ( $cover && $cover !== 'none' ) {
		...
		if ( $cover === 'rev' && $cover_rev ) {
			...
			//if ( $slider && ! Xtra_Theme::contains( $slider, '[rev_slider alias=' ) && ! isset( $_GET['vc_editable'] ) ) {
				...
				//	echo '<span>' . esc_html( Xtra_Strings::get( 'slider_placeholder_wpb' ) ) . '</span>';
				//} else {
